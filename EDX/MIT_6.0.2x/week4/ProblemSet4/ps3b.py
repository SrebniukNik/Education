...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    flue = [virus] * numViruses
    virus_not_treated = [0] * 300
    virus_treated = [0] * 300
    for trial in range(numTrials):
        patient = TreatedPatient(flue, maxPop)
        for step in range(300):
            patient.update()
            virus_not_treated[step] += float(patient.getTotalPop())
            virus_treated[step] += float(patient.getResistPop(['guttagonol']))
            if step == 149:
                patient.addPrescription('guttagonol')
                # print virus_treated, virus_not_treated

    for step in range(300):
        virus_not_treated[step] /= numTrials
        virus_treated[step] /= numTrials
    # print virus_not_treated
    # print virus_treated
    # pylab.plot(virus_not_treated, 'ro', label='ResistantVirus simulation')
