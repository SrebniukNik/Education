...
def BFSWithGenerator(start, end, q = []):
    ...
    while len(q) != 0:
        tmpPath = q.pop(0)
        print "tmpPath ", [path.label for path in tmpPath]
        lastNode = tmpPath[len(tmpPath) - 1]
        print "lastNode ", lastNode
        if lastNode.label == end.label:
            print "Returning ", [path.label for path in tmpPath]
            return tmpPath
        for shift in shiftDict[lastNode.spot]:
            print "Shift", shift, "shiftDict", shiftDict[lastNode.spot], "lastNode.spot ", lastNode.spot
            new = lastNode.transition(shift)
            print "new shift", new
            if notInPath(new, tmpPath):
                newPath = tmpPath + [new]
                print "Apending newPath to queue", [path.label for path in newPath]
                q.append(newPath)
    return None
...
path_BFSWithGenerator = BFSWithGenerator(test1, goal)
print "BFSWithGenerator", len(path_BFSWithGenerator)

# path_DFSWithGenerator = DFSWithGenerator(test1, goal)
# print "path_DFSWithGenerator", len(path_DFSWithGenerator)

# path_DFSWithGeneratorShortest = DFSWithGeneratorShortest(test1, goal)
# print "BFSWithGenerator", len(path_DFSWithGeneratorShortest)
